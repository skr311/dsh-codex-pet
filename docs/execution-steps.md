...
> M6（设置中调整宠物大小）已完成；M7（宠物默认初始位置改为左下角·侧边栏边上）已完成并验证通过；M8（DSH STORE 精确兼容声明）已完成
...
### M8 DSH STORE 精确兼容声明（✅ 完成）

- 原因：DSH STORE 最近三版兼容窗口要求插件 manifest 提供精确的 `dsh.compatibility.dshReleases` 记录。
- 插件版本提升至 `0.3.1`，补充 Node.js 与 DSH 兼容范围。
- 当前实际运行环境 DSH `0.1.5-rc.2` 声明为 `compatible`。
- 未验证的 `0.1.6-alpha.1`、`0.1.6-alpha.2` 声明为 `unknown`，不扩大未经验证的兼容承诺。
- 本次不涉及运行时代码修改。
...
| 版本 | 日期 | 说明 |
| --- | --- | --- |
| v0.7 | 兼容性 | M8 DSH STORE 整改：v0.3.1 精确声明 DSH 0.1.5-rc.2 兼容，两个未验证的 0.1.6 alpha 版本保持 unknown |
